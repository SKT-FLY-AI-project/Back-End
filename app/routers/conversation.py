...
from app.crud.chat_crud import get_or_create_conversation, save_message, get_conversation_history, get_user_conversations, delete_conversation_history
from app.services.llm_service import generate_vts_response
...
async def save(
    ...
    try:
        ...
        # artist = chat_data.artist
        # rich_description = chat_data.rich_description
        # conversation_id = chat_data.conversation_id
        
        # 대화 얻기 또는 생성
        conversation = await get_or_create_conversation(
            db, userid, image_url, title, artist, rich_description, None
        )
        
        return {"conversation_id": conversation.id, "message": "감상이 저장되었습니다."}
...
async def delete_conversation_detail(
    conversation_id: str,
    userid: str,
    db: AsyncSession = Depends(get_db)
):
    try:
        ...
        # 메시지 히스토리 조회
        result = await delete_conversation_history(db, conversation_id)
